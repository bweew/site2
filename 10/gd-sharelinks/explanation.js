...
$(document).ready(function() {
  ...
  if (explanationDiv.length > 0) {
    ...
    explanationDiv.html(`
      ...
      <p>inputing the url and title for links inside google forms also using shortcuts app and webhooks with integromat to send the data. then using app script that checks if the csv exists, if it doesnt then creates it. making sure to strip any commas replacing them with tabs. Also, when inputing from forms it auto creates a timestamp column which i needed to remove as well as rows a1:c1. they need to be deleted also by app script to not screw up my formatting. Installed google drive on my mac to sync the csv with my git repo, made a shortcut that checks if the modified date is less than a minute then runs a script to overwrite the csv in my git repo with the changes before pushing them. </p>
